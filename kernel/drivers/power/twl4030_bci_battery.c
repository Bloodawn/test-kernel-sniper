...
*/
	switch (psp) {
		case POWER_SUPPLY_PROP_STATUS:
			val->intval = di->charge_status;
			if(di->voltage_uV > FORCE_FULL_CHARGE_VOLTAGE_LEVEL) {		// QM Requested for Dummy Battery Test (LGLS855)
		...
		case POWER_SUPPLY_PROP_TEMP:
			if(di->battery_present == 0)		// No Battery or Dummy Battery
				val->intval = 200;
	                ...
	                val->intval = di->temp_C - 40;	// -40 is LGLS855 only value
		...
		case POWER_SUPPLY_PROP_CAPACITY:
			...
			if(di->charge_rsoc == POWER_SUPPLY_TYPE_MAINS ||
			   di->charge_rsoc == POWER_SUPPLY_TYPE_USB) {	// When Charging Mode
				...
				} else if(di->voltage_uV > FORCE_FULL_CHARGE_VOLTAGE_LEVEL) {	// QM Requested for Dummy Battery Test (LGLS855)
					val->intval = 100;
				}
			}
